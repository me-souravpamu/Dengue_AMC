...
cases_no_intervention = zeros(numel(years_forecast),size(Phat,1));

parfor k = 1:size(Phat,1)
    cases_no_intervention(:,k) = No_intervention_SEAIR(Phat(k,:),years_forecast);
end

%% ===== BAR-WISE (YEAR-WISE) DISTRIBUTION PLOT =====
save forecast_cases_2026_2028_without_intervention.mat cases_no_intervention

nY = numel(years_forecast);
cases_forecasts = cases_no_intervention';
